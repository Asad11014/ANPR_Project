...
def process_video_with_license_plate(video_path, results_path, output_path, authorised_plates):
    ...
    while ret:
        ...
        if ret:
            ...
            for row_indx in range(len(df_)):
                ...
                if license_plate_number not in authorised_plates:
                    draw_border(frame, (int(car_x1), int(car_y1)), (int(car_x2), int(car_y2)), (0, 0, 255), 25,
                            line_length_x=200, line_length_y=200)
                    cv2.putText(frame,
                            f"Unauthorised {license_plate_number}",
                            text_position,
                    ...
                    draw_border(frame, (int(car_x1), int(car_y1)), (int(car_x2), int(car_y2)), (0, 255, 0), 25,
                            line_length_x=200, line_length_y=200)
                    cv2.putText(frame,
                                f"Authorised {license_plate_number}",
                                text_position,
                                cv2.FONT_HERSHEY_SIMPLEX,
                                0.5,  # Font scale
